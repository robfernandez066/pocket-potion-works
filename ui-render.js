...
(() => {
  ...
  const narrativeDeliveryMarkup = detail => detail ? `${portraitMarkup(detail.customerId)}<div class="narrative-copy"><p class="eyebrow">${detail.kicker}</p><h2>${detail.title}</h2><p>${detail.body}</p><small>${detail.footer}</small></div>` : "";
