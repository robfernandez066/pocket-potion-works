...
const weatherMarkup = '<div class="workshop-weather-window is-clear" aria-hidden="true"><span class="weather-pane" aria-hidden="true"></span><span class="weather-moon" aria-hidden="true"></span><span class="weather-rain" aria-hidden="true"></span></div>';
assert.equal((html.match(/class="workshop-weather-window is-clear" aria-hidden="true"/g) || []).length, 1, "Weather needs one initially clear decorative window");
assert.ok(html.includes(weatherMarkup), "Weather layers must remain decorative and aria-hidden");
assert.doesNotMatch(weatherMarkup, /<(?:button|input|a|label|output|p|small|strong)\b/, "Weather must not introduce controls or copy");
assert.ok(style.includes(".workshop-weather-window{position:absolute;z-index:1;right:110px;top:10px;width:40px;height:56px") && style.includes(".workshop-weather-window.is-rain") && style.includes(".weather-rain") && style.includes("transition:background .25s"), "Weather needs exact collision-safe geometry, two stationary states, and a sub-300ms transition");
assert.ok(style.includes(".workshop-weather-window,.weather-moon,.weather-rain{transition:none!important}"), "Reduced motion must remove weather transitions");
const weatherStart = app.indexOf("const WORKSHOP_WEATHER_CADENCE_MS = 24000;");
const weatherEnd = app.indexOf("const CAULDRON_STIR_COPY", weatherStart);
const weatherPath = app.slice(weatherStart, weatherEnd);
assert.ok(weatherStart >= 0 && weatherEnd > weatherStart, "Weather must remain in one module-local presentation seam");
assert.ok(weatherPath.includes('let workshopWeather = "clear";') && weatherPath.includes('let workshopWeatherInterval = null;'), "Weather must begin clear with session-local cadence state");
assert.ok(weatherPath.includes('activeView() === "workshop" && !document.hidden') && weatherPath.includes("setInterval(() =>") && weatherPath.includes("}, WORKSHOP_WEATHER_CADENCE_MS);"), "Weather cadence must run only for a visible Workshop at exactly 24000ms");
assert.ok(weatherPath.includes("if (workshopWeatherInterval) return;") && weatherPath.includes("clearInterval(workshopWeatherInterval)") && weatherPath.includes('workshopWeatherInterval = null;'), "Weather must own at most one interval and clean up immediately when ineligible");
assert.ok(weatherPath.includes('workshopWeather = workshopWeather === "clear" ? "rain" : "clear";') && weatherPath.includes("renderWorkshopWeather();"), "Weather must alternate locally without gameplay state");
assert.doesNotMatch(weatherPath, /\b(?:Logic|state|save|storage|audio|sound|music|analytics|fetch|XMLHttpRequest|WebSocket|EventSource|geolocation|Date|location|navigator|reward|toast|feedback)\b/i, "Weather must not use gameplay, persistence, audio, network, location, or feedback paths");
assert.ok(app.includes("syncWorkshopWeatherCadence();\n  renderCauldronStirControls();") && app.includes('window.addEventListener("pageshow", syncWorkshopWeatherCadence);') && app.includes('document.addEventListener("visibilitychange", () => {\n  syncWorkshopWeatherCadence();') && app.includes("renderWorkshopWeather();\nsyncWorkshopWeatherCadence();"), "Weather cadence must synchronize on view changes, visibility, and initial load without ordinary rerender ownership");
assert.ok(!app.slice(weatherStart, weatherEnd).includes("addEventListener"), "Weather must not add interactive controls or focus behavior");
