...
(async () => {
  stores.set("ppw-shell-v57", new Map());
  const install = lifecycleEvent();
  listeners.install(install);
  await install.done();
  const activeCache = [...stores.keys()].find(name => name !== "ppw-shell-v57");
  assert.equal(activeCache, "ppw-shell-v58", "install must rotate to the actionable ordinary-order shell cache");
