...
(async () => {
  stores.set("ppw-shell-v52", new Map());
  const install = lifecycleEvent();
  listeners.install(install);
  await install.done();
  const activeCache = [...stores.keys()].find(name => name !== "ppw-shell-v52");
  assert.equal(activeCache, "ppw-shell-v53", "install must rotate to the achievement timing shell cache");
