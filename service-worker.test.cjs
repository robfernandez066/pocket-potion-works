...
(async () => {
  stores.set("ppw-shell-v54", new Map());
  const install = lifecycleEvent();
  listeners.install(install);
  await install.done();
  const activeCache = [...stores.keys()].find(name => name !== "ppw-shell-v54");
  assert.equal(activeCache, "ppw-shell-v55", "install must rotate to the Masterwork Alcove shell cache");
