...
(async () => {
  stores.set("ppw-shell-v59", new Map());
  const install = lifecycleEvent();
  listeners.install(install);
  await install.done();
  const activeCache = [...stores.keys()].find(name => name !== "ppw-shell-v59");
  assert.equal(activeCache, "ppw-shell-v60", "install must rotate to the content-catalog shell cache");
