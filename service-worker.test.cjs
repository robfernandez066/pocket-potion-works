...
(async () => {
  stores.set("ppw-shell-v58", new Map());
  const install = lifecycleEvent();
  listeners.install(install);
  await install.done();
  const activeCache = [...stores.keys()].find(name => name !== "ppw-shell-v58");
  assert.equal(activeCache, "ppw-shell-v59", "install must rotate to the bounded-Stardust shell cache");
