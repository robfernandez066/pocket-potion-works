...
(async () => {
  stores.set("ppw-shell-v53", new Map());
  const install = lifecycleEvent();
  listeners.install(install);
  await install.done();
  const activeCache = [...stores.keys()].find(name => name !== "ppw-shell-v53");
  assert.equal(activeCache, "ppw-shell-v54", "install must rotate to the Request Mix shell cache");
