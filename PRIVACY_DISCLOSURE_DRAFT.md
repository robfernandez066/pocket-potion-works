...
Pocket Potion Works stores a **local gameplay save** in the browser so progress can persist. It separately stores **local consent and simulated-commerce receipt state** used only by fake prototype adapters, plus a **local sound preference**. These records are versioned and treated as untrusted input on reload.
...
Resetting game data from the Journal removes the local gameplay save. It does not currently remove the separate sound or platform-prototype preferences; a production privacy UX must explicitly cover clearing all local records. Browser/site-data controls can remove all three namespaces.
