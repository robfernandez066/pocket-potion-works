...
- Service-worker cache generation is `ppw-shell-v37`. The install shell contains all code, eight sound effects, five approved sprite designs, and the optimized Aurora animation sheet; the three large music tracks stream from the same origin on demand so they do not block installation.
