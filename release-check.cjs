...
for (const file of [...runtimeFiles, ...runtimeAssets, ...streamedAssets, ...imageAssets, ...releaseDocs, "release-budgets.json", "release-browser-evidence.json", "fixtures/saves/legacy-pre-release-v1.json", "fixtures/saves/future-version-v7.json", "fixtures/rollback/game-save-reader-v1.cjs", "fixtures/rollback/game-save-reader-v2.cjs", "fixtures/rollback/game-save-reader-v3.cjs", "fixtures/rollback/game-save-reader-v4.cjs", "fixtures/rollback/game-save-reader-v5.cjs"]) assert.ok(fs.existsSync(file), `required release file missing: ${file}`);
...
assert.match(text["game-logic.js"], /const SAVE_VERSION = 6/);
assert.match(text["service-worker.js"], /const CACHE = `\$\{CACHE_PREFIX\}v39`/);
