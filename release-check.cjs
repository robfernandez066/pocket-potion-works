...
assert.match(text["service-worker.js"], /const CACHE = `\$\{CACHE_PREFIX\}v68`/);
...
assert.equal(total, 18370805, "Task 45 runtime inventory must match the approved exact total");
assert.equal(budgets.totalRuntimeBytes - total, 5629195, "Task 45 runtime inventory must retain the approved headroom");
