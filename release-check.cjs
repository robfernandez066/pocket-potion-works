...
assert.match(text["service-worker.js"], /const CACHE = `\$\{CACHE_PREFIX\}v77`/);
...
assert.equal(total, 18411908, "Task 63 runtime inventory must match the window-weather shell");
assert.equal(budgets.totalRuntimeBytes - total, 5588092, "Task 63 runtime inventory must retain the approved headroom");
...
assert.doesNotMatch(text["service-worker.js"], /v76/, "Task 63 must retire the prior shell cache identity");
const task38RuntimeBytes = 23993911;
assert.ok(normalizedReleaseBytes("app.js") <= 78000, "app.js must retain Task 63 weather-cadence headroom");
